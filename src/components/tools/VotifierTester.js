            ...
            <form onSubmit={form.handleSubmit}>
                <div className="flex flex-col gap-5 lg:flex-row">
                    <div className="basis-1/4">
                        ...
                        <select className="block select" id="version" defaultValue={form.values.version} onChange={form.handleChange} onBlur={form.handleBlur} disabled={form.isSubmitting}>
                            <option value="1">Votifier 1</option>
                            <option value="2">Votifier 2</option>
                        </select>
                        {
                            form.errors.version
                                ? <p className="mt-1 text-sm text-red-500">{form.errors.version}</p>
                                : null
                        }
                    </div>
                    <div className="basis-1/4">
                        ...
                        {
                            form.errors.host
                                ? <p className="mt-1 text-sm text-red-500">{form.errors.host}</p>
                                : null
                        }
                    </div>
                    <div className="basis-1/4">
                        ...
                        {
                            form.errors.port
                                ? <p className="mt-1 text-sm text-red-500">{form.errors.port}</p>
                                : null
                        }
                    </div>
                    <div className="basis-1/4">
                ...
                <div className="mt-5">
                    ...
                    {
                        form.errors.token
                            ? <p className="mt-1 text-sm text-red-500">{form.errors.token}</p>
                            : null
                    }
                </div>
                <button type="submit" className="flex justify-center w-full mt-5 button" disabled={form.isSubmitting || !form.isValid}>
                    {
                        form.isSubmitting
                ...
                {
                    form.status?.success
                        ? <p className="mt-3 text-green-500">Successfully sent a vote to the specified server.</p>
                        : form.status?.error
                            ? <p className="mt-3 text-red-500">{form.status.error}</p>
                            : null
                }
            </form>
        </div>
